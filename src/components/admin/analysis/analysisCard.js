...
const Card = ({ label, date, percent, money, unit, icon, onClick  }) => {
  ...
  return (
    <div onClick={onClick} className="border rounded-lg mx-2 w-2/6 shadow-md pl-4 pr-2 py-4">
      <h2 className="font-bold">{label}</h2>
      <div className="flex items-center">
      {icon && <span className="my-3">{icon}</span>}
      <p className="ml-3 text-sm font-semibold">{money}<span className="text-xs"> {unit} </span></p>
        <div
