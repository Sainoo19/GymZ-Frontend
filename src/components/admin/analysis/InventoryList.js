          ...
          <table className="w-full border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border p-2">Tổng số lượng</th>
            ...
            <tbody>
              {currentItems.map((item) => (
                <tr key={item.productId} className="text-center border-b">
                  <td className="border p-1 text-sm">{item.name}</td>
                  <td className="border p-2 text-sm">{item.category}</td>
                  <td className="border p-2 font-semibold text-blue-600">{item.totalStock}</td>
                  <td className="border p-2  text-sm">
                    {item.variations.map((variant, index) => (
                      <div key={index} className="text-sm text-gray-700">
                        {variant.theme} - {variant.category}: {variant.stock} cái
                      </div>
                    ))}
                  </td>
                </tr>
