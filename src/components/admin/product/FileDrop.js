...
export function FileDrop({
    ...
    if (cropper) {
      ...
      if (croppedCanvas) {
        croppedCanvas.toBlob(async (blob) => {
          ...
          // Thêm ảnh vào danh sách với tiến trình ban đầu là 0%
          setFiles((prevFiles) => [
            ...prevFiles,
            { url: croppedUrl, name: uniqueFileName },
          ]);
          setImages((prevUrls) => [...prevUrls, croppedUrl]);
          setUploadProgress((prev) => ({
            ...prev,
            [uniqueFileName]: 0, // Bắt đầu với 0%
          }));
  
          // Tiến hành upload lên Firebase
          await uploadImageToFirebase(blob, uniqueFileName);
  ...
  const uploadImageToFirebase = async (image, uniqueFileName) => {
    try {
      ...
      uploadTask.on(
        ...
        async () => {
          ...
          // Cập nhật URL Firebase khi upload xong
          setFiles((prevFiles) =>
            prevFiles.map((file) =>
              file.name === uniqueFileName ? { ...file, url: downloadURL } : file
            )
          );
  
          setImages((prevUrls) =>
            prevUrls.map((url) =>
              url === uniqueFileName ? downloadURL : url
            )
          );
  
          // Xóa trạng thái tiến trình sau khi hoàn thành
          setUploadProgress((prev) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className=" block mt-4 bg-gray-300 w-11/12 h-72 items-center rounded-2xl">
        {selectedAvatar ? (
          <img
            src={selectedAvatar}
            alt="Selected Avatar"
      ...
      {showCropper && previewImages.length > 0 && (
        <div className="mt-4 w-full flex flex-col items-center">
          ...
          <div className="flex w-1/2 mt-2  justify-between">
            ...
            <button
              className="px-4 py-2 border border-red-600 text-primary rounded-lg"
  ...
  >
    <div className="flex items-center w-full p-1 rounded-lg bg-gray-300">
      <input
        type="radio"
        name="avatar"
        value={file.url || ""}
    ...
    {/* Hiển thị thanh tiến trình nếu file đang upload */}
